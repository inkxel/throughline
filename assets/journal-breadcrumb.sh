#!/usr/bin/env bash
# Knowledge-layer — journal breadcrumb hook (PostToolUse on Bash).
#
...
# nudge the agent to write the WHY while it's fresh. The commit is the
# "larger move" marker (see knowledge/CLAUDE.md, Rule 1). HEAD-comparison means
# failed/blocked commits (e.g. a gitleaks reject) never breadcrumb, and we never
# double-write.
#
